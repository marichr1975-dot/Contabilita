...
struct ContabilitaRootView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $mostraPDF) {
            // FIX: PDFImportatiView ora richiede anche Archivio.
            ...
            ScrollView {
                VStack(spacing: 28) {
                    ...
                    LazyVGrid(columns: [GridItem(.flexible(), spacing: 22), GridItem(.flexible(), spacing: 22)], spacing: 22) {
                        dashboardButton("NUOVA BOLLETTA", icon: "plus.circle.fill", tint: .green) { nuovaBolletta = true }
                        dashboardButton("MODIFICA BOLLETTA", icon: "pencil.circle.fill", tint: .blue) { modificaBolletta = true }
                        dashboardButton("DATI ANALIZZATI", icon: "chart.bar.fill", tint: .purple) { mostraDatiAnalizzati = true }
                        dashboardButton(pdfTransfer.files.isEmpty ? "IMPORTA FILE AZIENDA" : "FILE AZIENDA  •  \(pdfTransfer.files.count)", icon: "doc.on.doc.fill", tint: .teal) {
                            pdfTransfer.ricarica(); mostraPDF = true
                        }
                    }
    ...
    private var header: some View {
        VStack(spacing: 10) {
            Image(systemName: "bag.fill").font(.system(size: isPad ? 62 : 52)).foregroundColor(.orange)
            Text("Contabilità").font(isPad ? .largeTitle : .title).fontWeight(.semibold)
        }.padding(.bottom, 8)
    }
    ...
    private func dashboardButton(_ title: String, icon: String, tint: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 14) {
                Image(systemName: icon).font(.system(size: isPad ? 32 : 27)).foregroundColor(tint)
                Text(title).font(isPad ? .title2 : .title3).fontWeight(.semibold).foregroundColor(.primary).lineLimit(2).minimumScaleFactor(0.75)
                Spacer(minLength: 8)
                Image(systemName: "chevron.right").font(.headline).foregroundColor(.secondary)
            }
